...
const Children = () => {
  ...
  const [currentChild, setCurrentChild] = useState({ id: null, first_name: '', last_name: '', date_of_birth: '', parent_user_id: null, branch_id: null });

  const handleAddNew = () => {
    setCurrentChild({ id: null, first_name: '', last_name: '', date_of_birth: '', parent_user_id: null, branch_id: null });
    setIsEditing(true);
  };
  ...
  const handleSave = async () => {
    const { id, first_name, last_name, date_of_birth, parent_user_id, branch_id } = currentChild;
    if (!first_name || !last_name || !date_of_birth || !parent_user_id || !branch_id) {
    ...
    try {
      if (id) {
        await updateItem(id, { first_name, last_name, date_of_birth, parent_user_id, branch_id });
      } else {
        await createItem({ first_name, last_name, date_of_birth, parent_user_id, branch_id });
      }
      setIsEditing(false);
    } catch (error) {
  ...
  const renderChildItem = ({ item }) => (
    <View style={styles.itemContainer}>
      <View style={styles.itemTextContainer}>
        ...
        <Text style={styles.itemDetails}>Parent: {item.parent_email}</Text>
      </View>
    ...
    return (
      <View style={styles.formContainer}>
        ...
        <Select
          ...
          options={parents.map(p => ({ label: `${p.first_name} ${p.last_name} (${p.email})`, value: p.id }))}
        />
        <Select
          placeholder="Select Branch..."
          value={currentChild.branch_id}
          onValueChange={(value) => setCurrentChild({ ...currentChild, branch_id: value })}
...
const styles = StyleSheet.create({
  ...
  list: { marginTop: 16 },
  itemContainer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFF', padding: 12, marginBottom: 8, borderRadius: 5, borderLeftWidth: 5, borderLeftColor: '#EF0107' },
